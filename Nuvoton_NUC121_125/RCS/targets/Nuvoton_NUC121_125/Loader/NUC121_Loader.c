...
#define LDROM_SIZE 4608
#define SPROM_SIZE 512

static libmem_geometry_t aprom_geometry[2] =
{
  [0] = { .size = PAGE_SIZE, .count = FLASH_SIZE / PAGE_SIZE, },
};
...
static struct config_struct fmc_config(void)
{
  ...
  /* perform ID command */
  FMC->ISPCMD = ISP_CMD_UINT32_READ;
...
int main(unsigned long param0)
{
  ...
  libmem_erase_impl(NULL, 0x0, sizeof(pattern), NULL, NULL);
  for (res = 0; res < sizeof(pattern); res++)
    pattern[res] = res;
  libmem_write_impl(NULL, (void *)0x1, pattern, sizeof(pattern));
#endif
