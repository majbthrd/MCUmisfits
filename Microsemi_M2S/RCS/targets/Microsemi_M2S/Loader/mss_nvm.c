...
#include "m2sxxx.h"
/* MODIFIED FROM ORIGINAL: removed "mss_assert.h" and "system_m2sxxx.h" #includes */
#include "mss_nvm.h"
...
static nvm_status_t request_nvm_access(uint32_t nvm_block_id)
{
     ...
     */
    /* MODIFIED FROM ORIGINAL: it is much simpler to just use worse-cast SystemCoreClock */
    timeout_counter = 142000000UL / 16u;
...
)
{
    ...
    *p_status = 0u;
    ...
    /* MODIFIED FROM ORIGINAL: bail gracefully upon erroneous offset rather than use ASSERT() macro */
